...
public class CpmServiceTest
{
    ...
    public void CalculateCriticalPath_ShouldHandleCompleteProjectNetwork()
    {
        ...
        foreach (TaskDTO criticalTask in result.CriticalTasks) Assert.AreEqual(0, criticalTask.Slack.TotalDays, 0.0001);
    }
    ...
    private void VerifyCriticalAndNonCriticalTasks(CpmResult result)
    {
        ...
        foreach (TaskDTO task in nonCriticalTasks) Assert.IsTrue(task.Slack.TotalDays > 0);

        List<TaskDTO> criticalTasks = result.AllTasks.Where(t => t.IsCritical).ToList();
        foreach (TaskDTO task in criticalTasks) Assert.AreEqual(0, task.Slack.TotalDays, 0.0001);
    }
    ...
    {
        for (int i = 0; i < expectedIds.Length; i++) Assert.AreEqual(expectedIds[i], result.CriticalPath[i].Id);
    }
